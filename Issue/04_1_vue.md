**面试官,你好,我叫肖刚,今年24岁,来自湖南郴州,从事前端开发已经3年了，使用的技术主要是vue全家桶,上一家公司主要负责的项目是后台系统的开发,我平时喜欢看一些掘金,知否等论坛,本人性格温和,能与同事友好相处,以上就是我简单的自己我介绍。。谢谢**

数据劫持,data上数据有变化,watch就会监视放到并放入update对象进行更新

响应式原理:watch调用get和set看数据是需要获取新的数据,还是许修修改数据实现数据的更新

v-model object.defineproperty方法调用set
...
- ![image-20220404155935775](C:\Users\22188\AppData\Roaming\Typora\typora-user-images\image-20220404155935775.png)
- Vue.config.optionMergeStrategies (自定义合并策略，比如可以合并所有组件同时拥有的一个值)
...
- Vue.nextTick(callback):  
  ...
  ![image-20220404183851851](C:\Users\22188\AppData\Roaming\Typora\typora-user-images\image-20220404183851851.png)
...
- 初始化阶段:
  ...
  ​     2.注意:禁止在created中输出书写大量逻辑相关代码

  ​    **3.beforeCreate和created之前,在做数据代理和数据劫持,同时将methods,computed中的属性,在实例对象上添加一份**

  3.beforeMount(渲染dom前)

  ​		1.created与beforeMount之前,

  ​				1.检查是否具有el属性,如果没有就等待$mount进行挂载

  ​				2.检查是否具有template属性,如果有他就会把template就会编译成render
  ...
  ​       注意:ssr服务端渲染没有mounted(https://juejin.cn/post/7046898330000949285)

  ​		2.操作真实DOM(只有mounted才能操作)
  ...
  ​      **3.beforeMount之后mounted之前,Vue会执行render函数用于创建虚拟DOM,同时通过虚拟DOM创建真实DOM,并且替换掉页面上的el元素,实现渲染**
...
- 捕获报错（错误边界）
  ...
  ​        Vue.config.errorHandler(不属于生命周期,属于Vue全局配置)
